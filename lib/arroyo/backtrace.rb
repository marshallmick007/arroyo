# Inspired by: https://github.com/philc/backtrace_shortener
#
...
module Arroyo
  module Backtrace
    def self.monkey_patch!
      ...
      Exception.class_eval do
        ...
        def backtrace
          ...
          Backtrace.filters.inject(backtrace) { |backtrace, filter| filter.call(backtrace) }
        end
      end
    end

    def self.show_only_app_exceptions!(rootdir=nil)
      unless Backtrace.using_app_filter
        Backtrace.filters.unshift( method(:only_app_exceptions) )
        Backtrace.backtrace_root = rootdir || Gem.dir
        Backtrace.using_app_filter = true
      end
    end

    def self.only_app_exceptions(backtrace)
      if Backtrace.backtrace_root == Gem.dir
        backtrace.reject { |line| line.include?(Backtrace.backtrace_root) }
      else
        backtrace.select { |line| line.include?(Backtrace.backtrace_root) }
      end
    end
    ...
    class << self
      attr_accessor :filters, :backtrace_root, :using_app_filter
    end
